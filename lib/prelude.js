...
function Product(list) {
	...
	var prod = 1;
	list.forEach(function(v,i) { if (v !== undefined) prod *= v; });
	return prod;
}
...
function _data(cube, url, args) {
	...
	if (data === FETCHING) throw "Fetch pending...";
...
function _binaryFetch(cube, url, args) {
	...
	if (data === undefined) {
		...
		oReq.onerror = function() {
			cache[url] = new Error("Timeout while fetching data");
			cube.recalculate();
		};
	...
	if (data === FETCHING) throw "Binary fetch pending...";
	if (data instanceof Error) throw data;
	return data;
}
...
function concat(list, rest) {
	if (rest) return Array.prototype.concat.apply([], arguments);
...
function text(list, fmt){
	function format(element){
		if(moment.isMoment(element)){
			return element.format(fmt);
		} else if (isNumeric(element)) {
			return numeral(element).format(fmt);
		}
...
function binaryfetch(url) {
	return ['Call', ['Symbol', '_binaryFetch'], ['Cube'], url];
}
